...
<template>
  <div ref="timerRef" class="flex flex-col justify-center items-center pt-4">
    ...
    <Transition name="slide-up">
      <div
        v-if="showSticky"
        class="fixed bottom-0 left-0 right-0 w-full z-50 bg-gradient-primary text-white px-4 py-3 shadow-lg flex justify-start lg:justify-between items-center gap-4 transition-transform duration-500"
      >
        <div class="flex items-center gap-2 text-xl font-bold">
          <img src="@/assets/images/product.jpg" class="w-11 h-auto" alt="t('hero.product_alt')" />
        </div>
        <div class="text-lg">
          <p class="text-white font-bold text-pretty">
            {{ t('timer.dialog_last') }}
            <Transition name="stock-change" mode="out-in" @after-enter="highlightStock">
              <span
                :key="stockKey"
                class="text-xl text-[#1f2937] font-bold"
                :class="{ highlight: isHighlighted }"
                >{{ stock }} {{ t('timer.pieces') }}</span
              >
            </Transition>
            {{ t('timer.sticky_at_this_price_product', { price: 111 }) }}
          </p>
        </div>
      </div>
    </Transition>
