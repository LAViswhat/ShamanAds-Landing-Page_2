...
<template>
  <section id="faq" class="py-12 bg-medical-light">
    <div class="container mx-auto px-4 z-20">
      <div class="text-center mb-8">
        <h2 class="text-4xl lg:text-5xl font-bold text-medical mb-6 text-pretty">
          {{ t('faq.title') }}
        </h2>
      </div>

      <div class="flex justify-center flex-col gap-6">
        <div v-for="question in questions" class="flex-1 mb-2k">
          <Accordion value="0" class="shadow-lg">
            <AccordionPanel>
              <AccordionHeader class="text-medical">{{ t(question.title) }}</AccordionHeader>
              <AccordionContent class="text-pretty">
                <p class="text-slate-500 pt-2">{{ t(question.description) }}</p>
              </AccordionContent>
            </AccordionPanel>
          </Accordion>
        </div>
